...
export default function FanArtAdmin({ onChange }) {
  ...
  async function loadPending() {
    ...
    try {
      ...
      if (!response || !response.ok) {
        setMessage('Failed to load entries — the FanArt database is unreachable.');
        setPendingEntries([]);
      } else {
  ...
  async function approveEntry(entry) {
    const response = await fetch(`${APEX_KV_URL}/fanart`, {
      method: 'POST',
      headers: getAdminHeaders(),
      body: JSON.stringify({ ...entry, approved: true }),
    }).catch(() => null);

    if (!response || !response.ok) {
      const errData = response ? await response.json().catch(() => ({})) : {};
      setMessage(`Failed to approve: ${errData.error || 'network error'}`);
    } else {
  ...
  async function deleteEntry(entry) {
    ...
    const response = await fetch(`${APEX_KV_URL}/fanart/delete`, {
      method: 'POST',
      headers: getAdminHeaders(),
    ...
    if (!response || !response.ok) {
      const errData = response ? await response.json().catch(() => ({})) : {};
      setMessage(`Failed to delete: ${errData.error || 'network error'}`);
    } else {
  ...
  async function createEntry(e) {
    ...
    if (!response || !response.ok) {
      const errData = response ? await response.json().catch(() => ({})) : {};
      setMessage(`Failed to create: ${errData.error || 'network error'}`);
    } else {
